...
import { Check, ChevronRight, CircleAlert, ImageUp, Minus, Plus, Sparkles, Trash2, Watch } from "lucide-react";
import { useAuth } from "@/_core/hooks/useAuth";
import { startLogin } from "@/const";
import { trpc } from "@/lib/trpc";
import { currentLocalDay, loadDailyCheckin } from "@/lib/dailyCheckin";
import { findPriorScreenshotUse, recordScreenshotUse, reviewSubmissionState, type ScreenshotUse } from "@/lib/screenTime";

type Habit = { id: string; name: string; mode: "build" | "reduce"; done: boolean };
...
export function HabitPlanner() {
  ...
  const [review, setReview] = useState<YesterdayReview | null>(null);
  ...
  const reviewYesterday = trpc.planner.reviewYesterday.useMutation({ onSuccess: (data) => { setReview(data.review as YesterdayReview); saveScreenshotUse(screenshotHash); }, onError: () => setMessage("The Screen Time review could not respond right now. Try again later.") });
  ...
  function requestPlan() { persist(); if (!requireSignIn()) return; if (priority.trim().length < 3) { setMessage("Write one honest priority for today first." ); return; } generate.mutate({ priority, habits: habits.map(({ name, mode }) => ({ name, mode })), stepTarget: stepTarget ? Number(stepTarget) : undefined, stepsSoFar: stepsSoFar ? Number(stepsSoFar) : undefined, sleepHours: sleepHours ? Number(sleepHours) : undefined }); }
