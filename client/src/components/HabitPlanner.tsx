...
import { slyNote, slyOnMark } from "@/lib/slyVoice";
...
/** The course day the learner is standing on, so the plan can serve it. */
function loadTodaysLesson() {
  try {
    const journal = JSON.parse(localStorage.getItem(COURSE_KEY) ?? "null") as { currentDay?: number } | null;
    const day = Math.min(100, Math.max(1, Math.round(journal?.currentDay ?? 1)));
    const lesson = getLesson(day);
    return { day: lesson.day, title: lesson.title, keyIdea: lesson.keyIdea, actionPrompt: lesson.actionPrompt, island: lesson.phase.island };
  } catch {
    return null;
  }
}
...
export function HabitPlanner() {
  ...
  const todaysLesson = useMemo(loadTodaysLesson, []);
  ...
  // Sly's read on the habits. He leads with one thing; the rest is offered
  // underneath rather than shouted, because a list of concerns is not advice.
  // The break he asks for is the floating fox's business, not this panel's.
